...
function normalizeProfile(raw = {}, source = 'custom_profile') {
  if (!profileIsUsable(raw, source)) return null
  ...
  return { title, artist, aliases: raw.aliases || [], key: normalizeKey(raw.key || raw.song_key || 'C'), capo: String(raw.capo || '0品'), bpm: Number(raw.bpm || 84), strumming: String(raw.strumming || '下 下上 空上 下上'), chords, progressions, source, matchLevel: raw.matchLevel || source, verified: true }
}
function findBuiltInProfile(title = '', artist = '') {
...
async function findCloudProfile(title = '', artist = '') {
  ...
  if (!titleKey) return null
  try {
    const exact = await songProfiles.where({ search_key: titleKey, status: _.neq('disabled') }).limit(3).get()
    const exactProfile = (exact.data || []).map((row) => normalizeProfile(row, 'cloud_song_profile')).find(Boolean)
...
function makeSection(name, chords, instruction) { const groups = splitBars(chords, 4); return { name, lines: groups.map((part, index) => ({ chordLine: bars(part), lyricLine: `${instruction}${groups.length > 1 ? ` · 第${index + 1}组` : ''}` })) } }
...
exports.main = async (event = {}) => {
  ...
  if (resolvedProfile) {
    ...
    return { code: 422, message: profileMissingMessage(webContext.title || event.title || '') }
  } else {

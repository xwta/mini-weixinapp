...
async function upsertProfile(input = {}) {
  ...
  if (existed.data?.[0]?._id) {
    ...
    return { ok: true, title: doc.title, action: 'updated' }
  }
  await profiles.add({ data: { ...doc, created_at: new Date() } })
  return { ok: true, title: doc.title, action: 'created' }
}
...
exports.main = async (event = {}) => {
  ...
  if (action === 'count') {
    ...
    return { code: 0, data: { total: total.total, verified: verified.total } }
  }
  if (action === 'seed') {
    const results = []

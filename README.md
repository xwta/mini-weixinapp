...
谱灵 AI 是一款面向吉他弹唱创作与练习的小程序。`cloudbase-native` 分支使用微信云开发原生能力实现前端、小程序云函数、云数据库、云存储和 CloudBase AI 能力的闭环。

核心目标：用户输入一句灵感或一段歌词，小程序生成可练习、可收藏、可发布的中文吉他弹唱谱。
...
| `ai-generate` | AI 写歌、AI 配和弦、保存 AI 曲谱、扣减额度 | `type=songwriting`、`type=chords` |
...
4. 绑定小程序 AppID
5. 开启云开发并选择正确环境
6. 预览登录、搜谱、AI 生成、评论、点赞收藏、消息、订单等核心链路
```
...
- [ ] 云函数已部署：`login`、`songs`、`ai-generate`、`ai-image`、`comments`、`notifications`、`interactions`、`discovery`、`orders`。
...
```text
cloudbase/cloudbaserc.json 的 envId 是否正确
云函数是否已部署
小程序端是否选择了同一个云环境
云函数运行时是否为 Nodejs18.15
```
